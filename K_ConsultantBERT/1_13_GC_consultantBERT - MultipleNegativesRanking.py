...
class ConsultantBERT:
    ...
    def train_model(self, pos_pairs, neg_pairs, output_path: str,
        ...
        # Initialize tracking variables exactly as in notebook
        MRR = []
        MRR_AT = 100
        training_start = "".join([c for c in str(datetime.now()).split('.')[0] if c.isdigit()])
        max_MRR = 0
        best_model_to_save = None  # Track best model
        ...
        for k in similarities.keys():
            ...
            # Save if best model exactly as in notebook
        ...
        return MRR, best_model_to_save  # Return both for consistency
